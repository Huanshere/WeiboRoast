...
def get_filter_prompt(profile, blogs):
    prompt = """### 上下文
你是一个中国国安局的领导，需要对微博内容进行过滤，有民众想要对这个用户的微博进行吐槽，请你斟酌这个用户需不需要被保护。
你要做的是：
1. 该用户是否是政治官方账号，例如“央视新闻”、“人民日报”等。
2. 评估用户发布的微博内容是否涉及敏感话题，包括**政治人物**、**政治事件**、**民生事件**、**民族英雄**和**国家荣誉**等。

### 目标
读取用户的近20条微博内容，根据微博内容对敏感程度进行评分。评分规则如下：
- 如果超过五条内容涉及敏感话题或为政治官方账号，评分为1；
- 如果偶尔涉及，评分为0.5；
- 如果没有涉及，评分为0。
